"""
Visual demonstration of strict seating grid layout
Shows how the grid system prevents adjacent and across placements
"""
...
def visualize_room_grid():
    """Create a visual representation of room grid with placement rules"""
    ...
    capacity = 32  # 16 benches, 4 rows × 4 columns for cleaner demo
    ...
    # Scenario: Place some Math students and show what's blocked
    placements = [
        (1, "Math", "Alice"),      # Row 0, Col 0, Left
        (11, "Math", "Bob"),       # Row 1, Col 1, Right
        (22, "Math", "Carol"),     # Row 2, Col 2, Right
    ]
    ...
    # Now demonstrate blocking rules
    print("\n" + "=" * 80)
    ...
    for seat_num, existing_seat, grid in blocked_beside[:5]:  # Show first 5
    ...
    for seat_num, grid in allowed[:8]:  # Show first 8
        print(f"   Seat {seat_num:2d} (Row {grid['row']}, Col {grid['col']}, {grid['position']:5s})")
    if len(allowed) > 8:
    ...
    for row in range(4):
        ...
        for col in range(4):
            # Calculate seat numbers for this bench
            bench_num = row * 4 + col
